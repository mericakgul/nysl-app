...
const Schedule = () => {
    ...
    const tableRowCells = (game) => columnTitles.map(title => <TableRowCell key={title}>{title === 'location' ? locationName(game) : game[title]}</TableRowCell>);
        ...
        <div>
            <h2>Fall Schedule</h2>
            {
                monthNumbers.map(monthNumber => {
                        return <Table key={monthNumber} className='table-striped table-hover'>
                                    <TableHeader>
                                    ...
                                    <TableBody>
                                        {games(monthNumber).map(game =>
                                            <TableRow key={game.id} navigateTo={() => navigate(`/gameDetails/${game.id}`)}>
                                                {tableRowCells(game)}
                                            </TableRow>
                                        )}
                                    </TableBody>
